...
class DetailsView extends React.Component {
  ...
  componentDidMount(){
    ...
    axios.get(`http://localhost/connection.php/StudentDetails/${id}`)
      .then(
        res => {
  ...
  setNewDetails(e){
    ...
    const id = this.props.match.params.id;
    axios.put(`http://localhost:3000/connection.php/${id}`, student)
    .then(res => {
  ...
  Delete(){
    const id = this.props.match.params.id;
    axios.delete(`http://localhost:3000/connection.php/${id}`)
      .then(res => {
        console.log(res);
